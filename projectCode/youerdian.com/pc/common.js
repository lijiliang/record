...
$(document).ready(function(){
	//initFormValidator();
	resetCartCount();
	$(document).on("click",".cusAlert #close",function() {
		$(".cusAlert").hide();
		$(".diaOverLay").hide();
	});
});
...
function initFormValidator()
{
	...
	$("#username").formValidator({onShow:"",onFocus:"",onCorrect:"验证通过"}).regexValidator({
		regExp:"^(?=.*[a-z]+).[a-zA-Z0-9_]{6,20}$",
		dataType:"string",
		onError:"6-20位字符，至少包含一位字母"
	});
	$("#password").formValidator({onShow:"",onFocus:"",onCorrect:"验证通过"}).regexValidator({
		regExp:"^[\\w.@~\!#$%^&*-]{6,32}$",
		dataType:"string",
		onError:"填写6-32个字符"
	});
	$("#password2").formValidator({onShow:"",onFocus:"",onCorrect:"密码一致"}).regexValidator({
		regExp:"^[\\w.@~\!#$%^&*-]{6,32}$",
		dataType:"string",
		onError:"填写6-32个字符"
	}).compareValidator({
...
{
	var refreshCartCount = window.setInterval(function(){
		$.getJSON("/simple/showCart",function(content){
			var count = content.count;
			$(".header_cart_prompt [name=mycart_count]").html(count);
			$(".sideMain [name=mycart_count]").html(count);
		});
	},60000);
}
...
function genOverLay()
{
	if($(".diaOverLay").length != 0) {
		return $(".diaOverLay");
	}
	var domHtml = [];
	domHtml.push("<div class='diaOverLay' style='display:none;position:fixed;width:100%;height:100%;background-color:black;opacity:0.7;top:0px;'></div>");
...
{
	var zindex = 10000;
	var px = ($(window).width() - 280)/2;
	var py = ($(window).height() - 150)/2;
	var overlay = genOverLay();
	if($(".cusAlert").length != 0) {
		overlay.css("z-index" , zindex + 1).show();
		$(".cusAlert").css({"z-index":zindex + 2,"left":px,"top":py}).find(".messCon").html(mess).end().show();
	}
	var domHtml = [];
	domHtml.push("<div class='cusAlert' style='position:fixed;'>");
	domHtml.push("<div class='alertTop'>");
	domHtml.push("<div class='messCon'>温馨提醒</div>");
	domHtml.push("</div>");
	domHtml.push("<div class='alertBottom'>")
	domHtml.push("<input type='button' id='close' value='确定'>");
	domHtml.push("</div></div>");
	overlay.css("z-index" , zindex + 1).show();
	$(domHtml.join("")).appendTo("body").css({"z-index":zindex + 2,"left":px,"top":py}).find(".messCon").html(mess).end().show();
}
//判断是否微信浏览器
function is_weixin(){
