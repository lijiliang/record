		...
		success:function(result){
			if(result.status){
				...
				for (var i = 0; i < 4; i++){
					...
					if (good){
						var str = [];
						str.push('<div class="w_imgOut" data-gid="'+good.gid+'" data-pid="'+good.periodCurrent+'"><a class="w_goods_img" data-gid="'+good.gid+'" data-pid="'+good.periodCurrent+'" href="javascript:void 0"><img id="img_'+good.gid+'" class="lazy'+id+'" data-original="'+imageGoodsPath+good.showImages.split(',')[0]+'" data-gid="'+good.gid+'" data-pid="'+good.periodCurrent+'"><noscript><img src="'+imageGoodsPath+good.showImages.split(',')[0]+'" alt=""></noscript></a></div>');
						...
						str.push('<dl class="w_rob"><dd><a class="w_slip" data-gid="'+good.gid+'" data-pid="'+good.periodCurrent+'" href="javascript:void 0">立即抢购</a><a title="加入购物袋" href="javascript:void(0);" class="w_slip2" data-gid="'+good.gid+'" data-pid="'+good.periodCurrent+'"></a></dd></dl><input type="hidden" id="priceArea_'+good.gid+'" value="'+good.priceArea+'"/>');
		...
		success:function(result){
			if(result.status){
				...
				$(result.notices).each(function(index, notice){
					str.push('<li><p class="yscrollfont"><a href="/footer/new_dynamic'+notice.id+'.html" title="'+notice.title+'">'+notice.title+'</a>');
			...
			if(result.status){
				...
				if(result.showsPage.total > 0){
					...
					for(var i=6-len; i>0; i--){
						$(".yCon8CenterinList2 ul").append('<li><img src="/static/img/front/index/shan_no.jpg"/></li>');
					}
...
var addAdFun = function(index, objs){
	if(objs){
		...
		$(objs).each(function(index, obj){
			str.push('<a target="_blank" href="'+obj.link+'" title="'+obj.description+'"><img src="'+imagePath+obj.img+'"></a>');
		});
		str.push('</div>');
			...
			if(result.status){
				...
				$(result.goods).each(function(index,goods){
					if($(".goods"+goods.gid+"_"+goods.periods).attr("class")!="yTimesLi goods"+goods.gid+"_"+goods.periods){
						...
						str.push('<dd class="yddName"><a href="/goods/goods'+goods.gid+'-'+goods.periods+'.html" target="_blank" title="'+goods.title+'">(第'+goods.periods+'期)'+goods.title+'</a></dd>');
...
var publishTimes=3;//动画揭晓完成后的揭晓次数  3次后将提示 "福彩中心通讯故障~请耐心等待"
//商品揭晓信息
			...
			if(result.status){
				...
				if(p == pid && goods.userWinCode){//返回来的商品详情期数不对应或没有云购码则揭晓失败
					...
					$(".goods"+gid+"_"+p).html(str.join(""));
				     ...
				     publishTimes--;//揭晓失败后将 减少揭晓次数 为0 是 则不会进行下次揭晓
				     if(publishTimes<0){
	...
	cart = '[{"buyPeriod":1,"client":1,"gid":' + gid
		...
		if(list.length>=30){
			...
			if (check == 0) {
				...
				+ gid+ ',"times":'+$("#priceArea_"+gid).val()+',"type":2}]';
			} else {
